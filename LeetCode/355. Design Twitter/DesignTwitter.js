...
Twitter.prototype.getNewsFeed = function (userId) {
  ...
  for (let i = this.tweets.length - 1; i >= 0 && feed.length < 10; i--) {
    if (
      ...
      (this.follows[userId] && this.follows[userId].includes(this.tweets[i].userId))
    )
...
Twitter.prototype.follow = function (followerId, followeeId) {
  ...
  if (!this.follows[followerId].includes(followeeId)) this.follows[followerId].push(followeeId)
}
...
Twitter.prototype.unfollow = function (followerId, followeeId) {
  ...
  if (index > -1) this.follows[followerId].splice(index, 1)
}
