...
var findLadders = function (beginWord, endWord, wordList) {
  ...
  let len = beginWord.length, result = [], tempBegin = beginWord
  const loop = (tempWordList, arr) => {
    for (let word of tempWordList) {
      let common = [], charIndex = 0, endIndex = 0, endCheck = [], alternative = ""
      for (let i = 0; i < len; i++) {
  ...
  let minLength = Math.min.apply(Math, result.map(function(arr) { return arr.length }))
